    ...
    <section className="relative bg-gradient-to-br from-orange-50 to-yellow-50 pt-40 sm:pt-48 pb-12 sm:pb-16 lg:pt-44 lg:pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          {/* Hero Content */}
          <div className={`space-y-6 lg:space-y-8 ${isAuthenticated ? 'lg:col-span-2 text-center' : ''}`}>
            <div className={`space-y-3 lg:space-y-4 ${isAuthenticated ? 'max-w-4xl mx-auto' : ''}`}>
              <h1 className={`text-3xl sm:text-4xl lg:text-6xl font-bold inter text-gray-900 leading-tight ${isAuthenticated ? 'text-center' : ''}`}>
                Discover
                <span className="text-primary block">Verified Merchants</span>
                in Nairobi CBD
              </h1>
              <p className={`text-base sm:text-xl text-gray-600 leading-relaxed ${isAuthenticated ? 'text-center max-w-3xl mx-auto' : ''}`}>
            ...
            {/* Trust Indicators */}
            <div className={`flex flex-wrap gap-3 sm:gap-6 ${isAuthenticated ? 'justify-center' : ''}`}>
            ...
            {!isAuthenticated ? (
              ...
              <div className="flex justify-center">
                <Link to="/merchants" className="inline-flex">
                  <Button size="lg" className="bg-primary hover:bg-primary-dark text-white rounded-[5px] px-8 py-4 text-lg">
                    Find Merchants
          ...
          {/* Search Feature - Only show when user is NOT authenticated */}
          {!isAuthenticated && (
          ...
          {/* Search Icon Button for Mobile - Only show when user is NOT authenticated */}
          {!isAuthenticated && (
            <button
